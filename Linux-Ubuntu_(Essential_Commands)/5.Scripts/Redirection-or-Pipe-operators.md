...
```
[root@localhost ~]# grep tuannguyen < tuannguyen.txt
tuan1
```
...
Tìm kiếm từ trong tệp :
```
[root@localhost ~]# grep tuan 0< tuannguyen.txt
tuan
```
...
Tìm kiếm từ trong tệp:
```
[root@localhost ~]# grep tuan tuannguyen.txt
tuan1
```
...
-rw-r--r--.  1 root root   63 Nov 26 15:23 tuannguyen.txt
...
Nối kết quả của lệnh "ls -al" trong tệp "tuannguyen.txt":
```
[root@localhost ~]# ls -la >> tuannguyen.txt
...
-rw-r--r--.  1 root root  702 Nov 26 15:32 tuannguyen.txt
-rw-r--r--.  1 root root  129 Dec 29  2013 .tcshrc
```
```
 ls -al 2>> file1 1>&2
...
```
[root@localhost ~]# cat tuannguyen.txt | grep tuan
tuan1
```
### 8. tee command
